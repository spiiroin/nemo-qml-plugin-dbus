...
#if 0
    arg.fd = 2;
    dbus_message_iter_append_basic(&sub, DBUS_TYPE_UNIX_FD, &arg);
    dbus_message_iter_append_basic(&sub, DBUS_TYPE_UNIX_FD, &arg);
    dbus_message_iter_append_basic(&sub, DBUS_TYPE_UNIX_FD, &arg);
#endif
...
service_inject_fake_args(DBusMessage *req)
{
    ...
    if( !strcmp(name, "FAKE1") )
        service_inject_variant_int32(&iter);
    else if(!strcmp(name, "FAKE2") )
        service_inject_dict(&iter);
    else if(!strcmp(name, "FAKE3") )
        service_inject_array(&iter);
    else if(!strcmp(name, "FAKE4") )
